load_dependencies <- function(){
  depend <- c("coda", "e1071", "png", "portalr", "runjags", "scoringRules", 
              "viridis")
...
load_models <- function(models, small = FALSE){
  ...
  names(out) <- paste0("mod", models_names)  
  out
}
...
fig1top <- function(){
  tiff("fig1top.tiff", width = 6, height = 4, units = "in", res = 1000)
  rc <- grey(0.6)

  par(mar = c(3, 1.75, 0, 0.75), fig = c(0, 1, 0.05, 0.7))
  ...
  mtext(side = 1, at = x4[n-1], cex = 0.8, line = 0.65, 
  ...
  mtext(side = 1, at = x4[n], cex = 0.8, line = 0.65, 
        expression(italic("t"[o])))

  points(fx, fy2, pch = 1, cex = 1.2, lwd = 2, col = grey(0,0.9))
  mtext(side = 1, at = fx[fn] + 0.03, cex = 0.8, line = 0.65, 
  ...
  text(x = mean(c(x4[n], fx[fn])), y = -0.25, "forecast horizon", 
       font = 3, cex = 0.6, xpd = TRUE)

  text(x = x4[n] - 0.15, y = -0.375, "forecast origin", 
       font = 3, cex = 0.6, xpd = NA)

  arrows(x4[n] - 0.065, -0.35, x4[n] - 0.015, -0.21, length = 0.05, 
         xpd = TRUE, lwd = 2)
  ...
  text(x = 0, y = 1, "(a)", cex = 1.1, xpd = TRUE)


  par(mar = c(0.5, 1.25, 0.5, 0), fig = c(0.475, 0.675, 0.71, 0.98), 
      new = TRUE)
  ...
  points(c(y3[n-1], y3[n-1] + 0.1), c(u[3], -1.9), type = "l", lwd = 2, 
         lty = 3, xpd = NA)
  mtext(side = 2, cex = 0.7, line = 0.25, "Density") 

  par(mar = c(0.5, 1.25, 0.5, 0), fig = c(0.05, 0.25, 0.71, 0.98), 
      new = TRUE)
  ...
  points(c(y3[1], y2[1] - 0.2), c(u[3], -1.7), type = "l", lwd = 2, lty = 3, 
         xpd = NA)
  mtext(side = 2, cex = 0.7, line = 0.25, "Density") 

  par(mar = c(0.5, 1.25, 0.5, 0), fig = c(0.75, 0.9755, 0.71, 0.98), 
      new = TRUE)
  ...
  points(c(fy2[1], fy2[1] - 0.5), c(u[3], -1.5), type = "l", lwd = 2, 
         lty = 3, xpd = NA)
  mtext(side = 2, cex = 0.7, line = 0.25, "Density") 
...
fig1bottom <- function(){
  ...
  tiff("fig1bottom.tiff", width = 6, height = 4, units = "in", res = 1000)
  rc <- rgb(0.8, 0.8, 0.8)
  ...
  text(-0.8, 2.55, "(b)", cex = 1.1, font = 1, xpd = NA)
  ...
  text(x = -0.25, y = 1.16, "Test origin = 14", cex = 0.5, xpd = NA)
  text(x = -0.25, y = 0.96, "True origin = 17", cex = 0.5, xpd = NA)
...
fig2top <- function(data_set, models){
  ...
  tiff("fig2top.tiff", width = 6, height = 4.5, units = "in", res = 1000)
  ...
  rectcol1 <- rgb(0.92, 0.92, 0.92, 1)
  rect(rectx1, -0.3, rectx2, 18, col = rectcol1, border = NA)
  rectx1 <- as.Date(moon_dates[501]) - 14
  rectx2 <- as.Date(moon_dates[512]) + 14
  rectcol1 <- rgb(0.85, 0.85, 0.85, 1)
  rect(rectx1, -0.3, rectx2, 18, col = rectcol1, border = NA)
  modcols1 <- viridis(3, 0.6, 0.7, 0)
  modcols2 <- viridis(3, 0.8, 0.7, 0)
  ...
  mtext(side = 2, line = 1.35, cex = 0.6, 
  ...
  par(fig = c(0.08, 0.38, 0.4, 0.65), mar = c(1.25, 1.5, 0, 0.5), new = TRUE)
  ...
  mtext(side = 2, line = 0.9, cex = 0.5, "Frequency")
  ...
  polygon(c(0.5, 12.5, 13.25, 13, 0.5),
          c(-1, -3.65, -3.65, -1, -1), col = rectcol1, border = NA,
         xpd = NA)
  ...
  for(i in 1:12){
    ...
    fx <- i
    xxa <- fx + dens$y[incl] * 0.3 + 0.2
    xxb <- fx - dens$y[incl] * 0.3 + 0.2 
    xx <- c(xxa, xxb[length(xxb):1])
    xx2 <- c(xx, xx[1])
    violiny <- c(dens$x[incl], dens$x[incl][length(dens$x[incl]):1])
    violiny2 <- pmax(0, c(violiny, violiny[1]))
    polygon(xx2, violiny2, border = modcols2[3], col = modcols1[3])
  }
  ...
  for(i in 1:12){
    ...
    incl <- which(dens$x < 15 & dens$y > 0.01)
    fx<-i
    xxa <- fx + dens$y[incl] * 0.3
    xxb <- fx - dens$y[incl] * 0.3 
    xx <- c(xxa, xxb[length(xxb):1])
    xx2 <- c(xx, xx[1])
    violiny <- c(dens$x[incl], dens$x[incl][length(dens$x[incl]):1])
    violiny2 <- pmax(0, c(violiny, violiny[1]))
    polygon(xx2, violiny2, border = modcols2[2], col = modcols1[2])
  }
  ...
  for(i in 1:12){
    ...
    violiny <- c(dens$x[incl], dens$x[incl][length(dens$x[incl]):1])
    violiny2 <- pmax(0, c(violiny, violiny[1]))
    polygon(xx2, violiny2, border = modcols2[1], col = modcols1[1])
  }

  points(1:12, y[302:313], pch = 16, col = rgb(1, 1, 1, 0.8))
  points(1:12, y[302:313], lwd = 2, cex = 1.1, col = rgb(0, 0, 0, 0.9))

  mtext(side = 3, c("RW", "AR(1)", "sAR(1)"), at = c(4, 6, 8), line = 0.2,
        cex = 0.6, font = 2, col = modcols2)
  ...
  axis(2, at = seq(0, 20, 10), las = 1, lwd = 0, line = -0.6,
       cex.axis = 0.6)
  mtext(side = 2, line = 0.8, cex = 0.55, 
  ...
  blank(xlim = c(0, 1), ylim = c(0, 1))
  text(0.22, 0.8, "(b)", xpd = TRUE, cex = 1.25)


  par(fig = c(0, 0.25, 0.65, 1), mar = c(0, 0, 0, 0), new = TRUE)
  blank(xlim = c(0, 1), ylim = c(0, 1))
  img <- readPNG("PP.png")
  rasterImage(img, 0, 0.5, 1, 0.84, col = 2)
  dev.off()
}
...
fig2bottom <- function(models, nbins = 10, last_in = 500){
  ...
  mod2 <- models$mod2 
  mod3 <- models$mod3
  modcols1 <- viridis(3, 0.2, 0.7, 0)
  modcols2 <- viridis(3, 0.8, 0.7, 0)

  tiff("fig2bottom.tiff", width = 6, height = 3.5, units = "in", res = 1000)
  ...
  for(i in 1:numods){
    x1 <- (i - 1) * 0.182 + 0.03
    x2 <- (i) * 0.182 + 0.03
    par(fig = c(x1, x2, 0.5, 1), mar = c(1.5, 0.75, 1, 0.25), new = TRUE)
    blank(bty = "L", ylim = c(0, 1.75), xlim = c(0.5, 10.5))
    rect(-1, -1, 12, 4, col = grey(0.92))
    box(bty = "L")
    points(PITs[i,], type = "h", lwd = 3, col = modcols2[i])
    ...
    axis(1, at = c(1, 10), labels = c(0, 1), lwd = 0, line = -1.25, 
         cex.axis = 0.5, xpd = NA)
    axis(2, at = seq(0, 1.75, 0.25), labels = FALSE, tck = -0.01)
    axis(2, at = seq(0, 1.75, 0.5), labels = FALSE, tck = -0.025)
    if(i == 2){
      mtext(side = 1, "Probability Integral Transform", cex = 0.75, 
             line = 0.5)
    }
    if(i == 1){
      mtext(side = 2, "Frequency", cex = 0.75, line = 0.75)
    }
  }
  ...
  points(max_crpss, crpss[max_crpss], cex = 1.75, pch = 16, lwd = 0, 
         col = modcols1[max_crpss])
  points(max_crpss, crpss[max_crpss], cex = 1.75, pch = 1, lwd = 1, 
         col = modcols2[max_crpss])
  points(crpss, pch = 16, cex = 0.9, col = modcols2)

  text(1:3, -2.59, mod_names2, xpd = TRUE, srt = 55, cex = 0.5, adj = 1)
  ...
  points(max_logss, logss[max_logss], cex = 1.75, pch = 16, lwd = 0, 
         col = modcols1[max_logss])
  points(max_logss, logss[max_logss], cex = 1.75, pch = 1, lwd = 1, 
         col = modcols2[max_logss])
  points(logss, pch = 16, cex = 0.9, col = modcols2)
  ...
  text(1:3, -2.545, mod_names2, xpd = TRUE, srt = 55, cex = 0.5, adj = 1)
  par(fig = c(0, 1, 0.92, 1), mar = c(0, 0, 0, 0), new = TRUE)
  blank(xlim = c(0, 1), ylim = c(0, 1))
  text(-0.01, 0.45, "(c)", xpd = TRUE, cex = 1.25)
  text(0.615, 0.45, "(d)", xpd = TRUE, cex = 1.25)
  ...
  for(i in 1:numods){
    x1 <- (i - 1) * 0.182 + 0.03
    x2 <- (i) * 0.182 + 0.03
    par(fig = c(x1, x2, 0, 0.5), mar = c(1.5, 0.75, 1, 0.25), new = TRUE)
    blank(bty = "L", ylim = c(0, 3.5), xlim = c(0.5, 10.5))
    rect(-1, -1, 12, 4, col = grey(0.85))
    box(bty = "L")
    points(eval_tabft_PIT[i,], type = "h", lwd = 3, col = modcols2[i])
    abline(h = 1, lty = 3, lwd = 2)
    text(1, 3.4, mod_names[i], cex = 0.75, adj = 0, xpd = TRUE)
    axis(1, at = seq(1, 10, 1), labels = FALSE, tck = -0.01)
    axis(1, at = seq(1, 10, 5), labels = FALSE, tck = -0.025)
  ...
  points(max_eval_tabft_c, eval_tabft_c[max_eval_tabft_c], cex = 1.5, 
         pch = 16, lwd = 0, col = modcols1[max_eval_tabft_c])
  points(max_eval_tabft_c, eval_tabft_c[max_eval_tabft_c], cex = 1.5, 
         pch = 1, lwd = 1, col = modcols2[max_eval_tabft_c])
  points(eval_tabft_c, pch = 16, cex = 0.9, col = modcols2)
  text(1:3, -3.3, mod_names2, xpd = TRUE, srt = 55, cex = 0.5, adj = 1)
  mtext(side = 2, "RPS", line = 1.1, cex = 0.6)
  ...
  points(max_eval_tabft_l, eval_tabft_l[max_eval_tabft_l], cex = 1.5, 
         pch = 16, lwd = 0, col = modcols1[max_eval_tabft_l])
  points(max_eval_tabft_l, eval_tabft_l[max_eval_tabft_l], cex = 1.5, 
         pch = 1, lwd = 1, col = modcols2[max_eval_tabft_l])
  points(eval_tabft_l, pch = 16, cex = 0.9, col = modcols2)
  text(1:3, -2.745, mod_names2, xpd = TRUE, srt = 55, cex = 0.5, adj = 1)
  mtext(side = 2, "Log Score", line = 1.15, cex = 0.6)
  axis(side = 2, las = 1, cex.axis = 0.5, tck = -0.05, labels = FALSE)
  ...
  blank(xlim = c(0, 1), ylim = c(0, 1))
  text(-0.01, 0.45, "(e)", xpd = TRUE, cex = 1.25)
  text(0.615, 0.45, "(f)", xpd = TRUE, cex = 1.25)
  dev.off()
}
